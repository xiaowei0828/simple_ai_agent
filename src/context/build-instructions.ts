...
- Use workspace-relative paths for project files, and the advertised absolute paths for skills and their index. Keep run_command cwd inside the workspace even when reading an external skill. For an explicit path, operate directly.
...
- Read small files with cat and line ranges with sed -n 'START,ENDp', or use Get-Content with Select-Object in PowerShell. Request relevant ranges instead of dumping large files. If output is truncated, read a smaller range; do not treat partial output as a complete file.
...
- Use run_command for file reads, directory listings, searches, builds, tests, version control, and other commands. Write the complete command using ${shell.displayName} syntax, and check its exit code and output. Prefer rg for text, rg --files for file discovery, and ls (Get-ChildItem in PowerShell) for directory contents. If rg is unavailable, use the active shell's alternatives.
