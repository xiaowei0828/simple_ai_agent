...
import type { ModelRequest, ModelStreamEvent } from "../src/core/types.js";
...
function createClient(
  ...
  options: Pick<ClientOptions, "maxRetries"> = {},
): OpenAI {
  return new OpenAI({
    apiKey: "test-key",
...
function messageResponse(id: string, text = "hello") {
  return {
    id,
    object: "response",
    created_at: 1,
...
describe("OpenAIModel", () => {
  ...
  it("captures the SDK request body and full parsed response without credentials", async () => {
    ...
    const transmitted = JSON.parse(transmittedBody) as Record<string, unknown>;
    ...
    expect(transmitted).not.toHaveProperty("stream");
    ...
    expect(serializedTraces).not.toContain("api-key-that-must-not-be-logged");
    expect(serializedTraces).not.toContain("must-not-be-logged");

    const serialModel = new OpenAIModel({ client, parallelToolCalls: false });
    await serialModel.respond(modelRequest({ instructions: "full instructions" }));
  ...
  it("streams reasoning and output deltas while returning the completed response", async () => {
    ...
    const eventStream = `${streamEvents.map(sse).join("")}data: [DONE]\n\n`;
  ...
  it("accepts a terminal streaming event even if the connection then terminates", async () => {
    ...
    const completedResponse = messageResponse("resp-terminal", "complete");
    ...
    const client = createClient(async () => {
      ...
      return new Response(new ReadableStream({
        start(controller) {
          controller.enqueue(encoder.encode(sse(completedEvent)));
    ...
    const result = await model.respond(modelRequest({ stream: true }));
  ...
  it("propagates a terminated response body without retrying", async () => {
    ...
    await expect(model.respond(modelRequest({
      stream: true,
      async onStreamEvent(event) { emitted.push(event); },
    ...
    const client = createClient(async () => {
      ...
      return new Response(new ReadableStream({
        pull(controller) {
          if (!sentDelta) {
            sentDelta = true;
            controller.enqueue(encoder.encode(sse(deltaEvent)));
            return;
          }
          controller.error(new TypeError("terminated"));
        },
  ...
  it.each([undefined, "low"] as const)("preserves reasoning effort (%s) when falling back without reasoning summaries", async (effort) => {
    ...
    const client = createClient(async (_input, init) => {
      ...
      return new Response(JSON.stringify(messageResponse(`resp-${requestCount}`)), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    });
    ...
    expect(transmittedBodies.map((body) => body.reasoning)).toEqual(effort
      ? [{ summary: "auto", effort }, { effort }, { effort }]
      : [{ summary: "auto" }, undefined, undefined]);
  });
  ...
  it("sends explicit reasoning effort none and does not treat effort errors as unsupported summaries", async () => {
    ...
    const client = createClient(async (_input, init) => {
      ...
      return new Response(
        JSON.stringify({ error: { message: "Unsupported value for reasoning.effort", type: "invalid_request_error" } }),
        { status: 400, headers: { "content-type": "application/json" } },
      );
    }, { maxRetries: 0 });
    const model = new OpenAIModel({ client });
    await expect(model.respond(modelRequest({ model: "test", instructions: "test", reasoningEffort: "none", reasoningSummary: "auto" })))
